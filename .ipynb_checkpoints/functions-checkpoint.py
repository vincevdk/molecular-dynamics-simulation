...
def calculate_time_evolution(vel, pos, force, potential_energy,kinetic_energy,drift_velocity):
    for v in range(1,Nt):   
        vel =  vel + h*force/2
        pos[v] = (pos[v-1] + h*vel) % L
        min_dis, min_dir = calculate_minimal_distance_and_direction(pos[v])
        force = calculate_force(min_dir, min_dis)
        vel = vel + h*force/2
        potential_energy[v] = calculate_potential_energy(pos[v], min_dis,min_dir, potential_energy[v])
        kinetic_energy[v] = calculate_kinetic_energy(kinetic_energy[v], vel)
        drift_velocity[v,:] = np.sum(vel,axis=0)
    return(pos,potential_energy,kinetic_energy,drift_velocity)
